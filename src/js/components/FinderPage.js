  ...
  initActions(){
    ...
    if(thisFinder.stage === 1){
      thisFinder.dom.gridContainer.addEventListener('click', function(event) {
        ...
        if(clickedElement.classList.contains(classNames.finder.gridItem)){
  ...
  markField(clickedElement){
    ...
    if(thisFinder.grid[clickedField.row][clickedField.col]) {
      ...
      if(clickedField.row === thisFinder.clickedGridOrderX[thisFinder.clickedGridOrderX.length - 1] && thisFinder.clickedGridOrderY[thisFinder.clickedGridOrderY.length - 1] === clickedField.col ){
        ...
        previouslyClickedElem = thisFinder.dom.gridContainer.querySelector('[data-col="' + thisFinder.clickedGridOrderY[thisFinder.clickedGridOrderY.length - 1] + '"]' + '[data-row="' + thisFinder.clickedGridOrderX[thisFinder.clickedGridOrderX.length - 1] + '"]' );


        if(previouslyClickedElem !== null){
          previouslyClickedElem.classList.replace(classNames.finder.gridItemClicked, classNames.finder.gridItemLastClicked);
        }
      }
    }
      ...
      previouslyClickedElem = thisFinder.dom.gridContainer.querySelector('[data-col="' + thisFinder.clickedGridOrderY[thisFinder.clickedGridOrderY.length - 2] + '"]' + '[data-row="' + thisFinder.clickedGridOrderX[thisFinder.clickedGridOrderX.length - 2] + '"]' );

      if (previouslyClickedElem !== null){
        previouslyClickedElem.classList.replace(classNames.finder.gridItemLastClicked, classNames.finder.gridItemClicked);
      }
    }
  }
